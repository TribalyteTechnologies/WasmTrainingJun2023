...
void Message::newMessage(MessageComposition messageComposition, Tribalyte department) {
    try {
        ...
        std::string departmentChosen = ((department == Tribalyte::tribalyteTech)?("Tech department"):("Ideas department"));
...
EMSCRIPTEN_BINDINGS(message) {
  class_<Message>("Message")
    ...
    enum_<Tribalyte>("Tribalyte")
    .value("Tech", Tribalyte::tribalyteTech)
    .value("Ideas", Tribalyte::tribalyteIdeas);
}
